...
class RTKLibGNSSManager(DataManager):
    ...
    def __init__(
        ...
        num_skip_rows : int, optional
            The number of header rows in the .pos file. If None, the number of rows is inferred automatically from the file (default).
    ...
    def load_sequence(self, include=None):
        """Load a sequence of multiple .pos files in the data directory.

        Parameters
        ----------
        include : list, optional
            Provide a list of file names to include. Expected format: 
            ['file001.pos', 'file002.pos', ...]. If None, all .pos files 
        ...
        for path, folders, files in os.walk(self.dir):
            for f in files:
                if f[-4:] == ".pos" and _include_file(f):
    ...
    def load_tracks(self, filename):
        ...
        # load .pos file
        df, meta_rtklib = self._read_pos_file(os.path.join(self.dir, filename))
        if df.shape[0] == 0:
            return []
    ...
    def _find_pos_header(self, filepath):
            ...
            for line in f:
                ...
                if line.startswith("%"):
                    ...
                    if ":" in line and not line.startswith("(") and not line.endswith(")"):
                        ...
                        pos_metadata[key.strip()] = value.strip()
                    else:
                        col_labels = line
                else:
                    break
        ...
        expected_columns = ['GPST', 'latitude', 'longitude']    # test the existance of the most signature column names to verify that this is a pos-file and that we found the header.
        for col in expected_columns:
            if col not in col_labels:
                raise ValueError(f"File '{filepath}' does not have the expected header of an RTKLib .pos file.")
            
        return num_rows_header, pos_metadata
    ...
    def _read_pos_file(self, filepath):
        ...
        metadata_rtklib : dict
            The RTKLib metadata from the header of the .pos file
        """

        if self.num_skip_rows is None:
            num_skip_rows, metadata_rtklib = self._find_pos_header(filepath)
        else:
            _, metadata_rtklib = self._find_pos_header(filepath)
            num_skip_rows = self.num_skip_rows
