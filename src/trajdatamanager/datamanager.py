...
class RTKLibGNSSManager(DataManager):
    ...
    def __init__(
        ...
        projection="utm",
        zone=31,
        reference_location="mean",
        num_skip_rows = None,
    ):
        super().__init__(pos_file_name)
    ...
    def load_tracks(self, filename):
        ...
        if self.reference_location == "mean":
            xref = np.mean(x)
            yref = np.mean(y)
            ...
            xref, yref = proj(self.reference_location[1], 
                              self.reference_location[0])
            x = x - xref
            y = y - yref

        t = df["Timestamp"]
        t = np.array([ti.to_pydatetime().replace(tzinfo=dt.UTC) - dt.timedelta(seconds=18) for ti in t])
        ddt = np.gradient([(ti-t[0]).total_seconds() for ti in t])

        # derive speed from position trajectory
        v_x = np.gradient(x) / ddt
        v_y = np.gradient(y) / ddt
        ...
        data = np.c_[x, y, psi, v, v_x, v_y]

        keys = ["x", "y", "psi", "v", "v_x", "v_y"]
        
        metadata = {'track_type': 'RTKLIBGNSSData',
                    'reference_location': [xref, yref],
        ...
        # t = t - t[0]
        # t = [ti.total_seconds() for ti in t]
...
class Track:
    ...
    def __init__(
        ...
        assert all(
            isinstance(ti, dt.datetime) for ti in t
        ), "Has to be datetime!"
        self.t = t
    ...
    def plot(self, axes=None, features=None, plot_over_timestamps=False, t_plot=None, **plot_kwargs):
        ...
        if t_plot is not None:
            ...
            for ax, feat  in zip(axes, features):
                ax.plot(to_finite(self[feat]), **plot_kwargs)
                ax.set_ylabel(feat)
